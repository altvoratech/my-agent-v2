...
- **Modelos Claude 5 no seletor** — `claude-opus-5` e `claude-sonnet-5` entram nos catálogos do web chat e do TUI, com janela de contexto de 1M cada. IDs conferidos na skill `claude-api` e ambos **verificados em execução real** contra a assinatura antes de entrar. O default do servidor segue `claude-sonnet-4-6`.
