...
- **Delegação pegajosa** — uma tool de delegação invocada num turno marca o turno *seguinte* como delegado. Corrige um falso positivo observado ao vivo no `auditor de referência externo`: uma `Skill` invocada no turno N é *executada* no N+1, e o auditor, que olha só a janela do turno, lia o N+1 como trabalho pesado sem delegação.
...
- **`docs/superpowers/`** — spec de desenho e plano de implementação do portão, escritos antes do código pelo fluxo `brainstorming → writing-plans → execução`. O spec documenta o mapa completo dos portes de um harness externo para este repo e o que fica de fora, com o princípio de portar o **mecanismo**, nunca a implementação.
