...
| 🚪 **Portão de delegação** | audita cada turno: leitura pesada sem delegar ao `explorer` sobe a um juiz haiku, que devolve a lacuna. Toda decisão é gravada em `delegation_audits` (SQLite) para calibração. Default = **observação** (grava sem bloquear); `DELEGATION_GATE=on` liga o bloqueio. ([detalhes](docs/portao-delegacao.md)) |
...
docs/             ADRs, documentação do portão de delegação e screenshots
```
